...
def just_eye(img, eye_points):                  #얼굴에서 눈만 추출
  ...
  marginx, marginy = w / 2, h / 2

  minx, miny = int(xx - marginx), int(yy - marginy)
  maxx, maxy = int(xx + marginx), int(yy + marginy)

  eye_rect = np.rint([minx, miny, maxx, maxy]).astype(np.int)
...
while cap.isOpened():
  ...
  if faces and startce==0: 
    if startcode<100:
        ...
        ps=pygame.mixer.Sound('deepstart.wav')
        ps.play()
        time.sleep(3)
        print("START DRIVE")
  ...
  for face in faces:
   ...
   if ws%5==0:
    ...
    eye_img, eye_rect = just_eye(gray, eye_points=shapes[36:42])
    ...
    cv2.rectangle(img, pt1=tuple(eye_rect[0:2]), pt2=tuple(eye_rect[2:4]), color=(255,255,255), thickness=2)
   

    cv2.putText(img, state, tuple(eye_rect[0:2]), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,255,255), 2)
